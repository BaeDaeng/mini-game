...
export const SYMBOLS = [
  ...
  { id: 'great_spirit', name: '🌬️ 대정령', rarity: 'rare', gold: 2, destroy: '(제나리스의 축복), (아그네스의 축복), (에키드나의 축복) 파괴시 골드 4개를 제공' },
  ...
  { id: 'vampire', name: '🧛 뱀파이어', rarity: 'rare', gold: 2, adjacent: '(프리스트)마다 (뱀파이어의 정수)를 추가' },
  ...
  { id: 'lucifer', name: '😈 루시퍼', rarity: 'special', gold: 4, adjacent: '모든 (축복) 계열이 (저주) 계열로 변화' },
  { id: 'gabriel', name: '🕊️ 가브리엘', rarity: 'special', gold: 4, destroy: '(임프), (가고일), (뱀파이어), (서큐버스) 파괴 시 천사 스택 누적', effect: '천사 스택이 4번 쌓이면 (미카엘)로 진화' },
  { id: 'demon_cult', name: '👁️ 마왕교', rarity: 'special', gold: 4, destroy: '(이단자) 파괴 시 25% 확률로 (가인의 저주)를 추가' },
  { id: 'cross', name: '✝️ 십자가', rarity: 'special', gold: 2, destroy: '모든 (저주) 계열 파괴 시 기본 제공 골드를 영구적으로 1개 더줌' },
  { id: 'altar', name: '🏛️ 제단', rarity: 'special', gold: 5, effect: '스핀마다 (축복)을 추가. 4번 스핀 후 자신을 파괴' },
  { id: 'archbishop', name: '📿 대주교', rarity: 'special', gold: 3, adjacent: '(아그네스/에키드나/루리티아의 축복) 마다 골드 1개를 제공' },
  ...
  { id: 'undead_king', name: '👑 언데드 킹', rarity: 'legendary', gold: 5, destroy: '(이단자), (신자), (수도원장), (강령술사) 파괴 시 (스켈레톤)을 추가' },
...
export const getDrawProbabilities = (stage) => {
  const legendary = Math.min(1 + stage * 0.5, 10);
  const special = Math.min(5 + stage * 1.5, 20);
  const rare = Math.min(20 + stage * 2, 40);
  const common = Math.max(100 - legendary - special - rare, 30);
  return { common, rare, special, legendary };
};

export const getRandomSymbol = (stage) => {
  const probs = getDrawProbabilities(stage);
